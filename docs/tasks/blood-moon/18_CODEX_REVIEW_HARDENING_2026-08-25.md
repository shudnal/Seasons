...
runtime-code head immediately before this checkpoint update: 5d89e2cd12a5701be05364835e548061597bf0b0
```
...
The handoff is serialized across live owners. If the ZDO owner changes while a relay to the previous owner is still in flight, the server does not issue the same accepted request to the new owner in parallel. It waits for the previous live peer to return an explicit success/failure acknowledgement; only a negative acknowledgement or loss of that peer makes the request eligible for relay to the newly resolved owner. This prevents ordinary network delay from turning ownership migration into duplicate vanilla `RPC_SpawnBoss` execution.

Acknowledgements are accepted only from the owner to which the current relay was sent. Cross-world stale pending entries are discarded.
...
The central `ZNet.OnDestroy` Blood Moon lifecycle cleanup explicitly clears pending offering requests. The cached `ZRoutedRpc` registration reference is deliberately retained, preserving the existing no-duplicate-registration invariant if runtime state resets while the same transport object remains alive.

Vanilla behavior used for this design was verified from `assemblies_combined`: `OfferingBowl.InitiateSpawnBoss` routes `RPC_SpawnBoss` to the bowl ZDO owner; `RPC_SpawnBoss` performs the owner/queue/`CanSpawnBoss` checks, schedules the delayed boss spawn, and only then sends the normal requester-side item-removal/feedback RPCs. The Blood Moon authority therefore serializes only the pre-dispatch ownership handoff and leaves the accepted vanilla delayed-spawn implementation intact.
...
The lower lease path was also re-audited. `BloodMoonSpawner.ReceiveLease` has no second server-only rejection and validates the same local owner session. `BloodMoonLeaseClientTime` intentionally skips remote-client monotonic rebasing on the server/listen host: host leases are authored and consumed in the same server `seasonState.GetTotalSeconds()` time domain, whereas remote clients rebase the received TTL to `Time.realtimeSinceStartup` to avoid independent clock-domain drift.
...
A related disconnect-ordering edge was reviewed against the authoritative combat contract and intentionally not changed in this pass. `04_COMBAT_PROGRESS_AND_RESOLUTION.md` states that an attributed projectile may still deal physical damage after its owner has `Exited`, but must not grant progress/skill credit after that exit. Simply accepting a death report for any now-terminal participant would therefore be wrong. Preserving a kill whose server-confirmed credit predates a later disconnect would require preserving the historical progress/share context, not a source-only receive-time exception; no partial semantic change was introduced without that evidence.
...
The runtime diff from the previous reviewed head `75e6d27cb47171ebd3fe409313e06ae4a0142dab` through runtime head `5d89e2cd12a5701be05364835e548061597bf0b0` is confined to Blood Moon runtime hardening files, with the checkpoint documentation updated separately. No release metadata is part of this hardening pass.

All five review threads from that Codex pass were answered and resolved before this checkpoint.

The OfferingBowl relay was additionally audited for normal ordering and lifecycle edges after the review fix: skipped/resolved events do not inherit the frozen cutoff, accepted requests have no arbitrary migration timeout, world teardown explicitly removes any remaining accepted request state, and one accepted request is never dispatched concurrently to two connected owners merely because ZDO ownership changed while the earlier ACK was still in flight.
...
- bowl ownership migration while the previous owner remains connected and its relay ACK is delayed;
- bowl ownership migration including a temporary ownerless/disconnected interval before completion;
- stale relay acknowledgement after another handoff decision;
...
- disconnect ordering around already observed lethal damage, with the post-exit no-progress rule preserved;
